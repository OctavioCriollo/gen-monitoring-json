...
bool save_SD_card(bool& save, bool& event, bool& SD_init, fs::FS &fs, const char* folder, const char* file, DynamicJsonDocument doc){
    if(!save)
        return false;
...
void RBS_monitoring(RBS& rbs){
    ...
    const char* date = dateToString(now).c_str();

    rbs.energy.status.setTimestamp(date);
    rbs.gen.status.setTimestamp(date);
    rbs.gen.tta.status.setTimestamp(date);
    rbs.powerSystem.battery.status.setTimestamp(date);
...
void print_msg(){
    ...
    Serial.printf("===========================\n");
    Serial.printf("* Energia --> %s\n",rbs.gen.ac.status.code());
    Serial.printf("* GEN --> %s\n",rbs.gen.status.code());
    Serial.printf("* TTA --> %s\n",rbs.gen.tta.status.code());
    Serial.printf("* BAT --> %s\n",rbs.powerSystem.battery.status.code());
...
void setup() {
    ...
    if(wifi_connect(ssid,password,5)){
        ...
        if(mqtt.connect(1)){
            ...
            mqtt.subscribe();
            mqtt.client.publish(mqtt_topic_pub,"Hello I am ESP32");
        }
...
void loop() { 
    ...
    if(isrEvent)
        if(wifi_check_connection(ssid,password,4)){
            if(mqtt.checkConnection(1)){
                Serial.printf("\nConection OK to %s",mqtt.server());
                mqtt.publish(isrEvent,rbs.toJson());
    ...
    Serial.printf("\nTimestamp GEN: %s",rbs.gen.ac.status.timestamp());
    Serial.printf("\nTimestamp Power: %s",rbs.powerSystem.ac.status.timestamp());

    save_SD_card(save_SD,isrEvent,SD_Initialized,SD,path_Folder,SD_File,rbs.toJson());
    /*=================================================================================*/
    isr_enable();
    print_msg();
    delay(1*DELAY_TIME);
